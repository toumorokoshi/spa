...
- Generated by `scripts/generate-kana-words.mjs` from [Hermit Dave’s Japanese frequency list](https://github.com/hermitdave/FrequencyWords) (MIT): every unique kana-only token of length ≥ 2 from the full corpus file (deduplicated by first occurrence), then sorted with `localeCompare(..., 'ja')` so the bundled list spans the syllabary evenly instead of truncating to the first _N_ by frequency (which skews toward early mora).
